  ...
  if (!user) {
    if (route === "comments_form") {
      userStatus.renderInstead = (
        <p id="commentLoggedOut">Log in to add comments</p>
      );
    } else {
      const message =
        route === "add_note_form" ? "add notes" : "view your profile";
      userStatus.renderInstead = (
    ...
    if (route === "comments_form") {
      ...
      userStatus.renderInstead = (
        <div className="text-center logMessage">{returnMessage}</div>
      );
    }
  } else if (user.status === "BLOCKED") {
    const returnMessage =
      "Your account has been blocked for spam or inappropriate content.";
    if (route === "comments_form") {
      userStatus.renderInstead = <p id="commentLoggedOut">{returnMessage}</p>;
    } else {
      userStatus.renderInstead = (
